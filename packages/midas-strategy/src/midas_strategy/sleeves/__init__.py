...
The liquidity tier is used by the cost model to estimate slippage and market impact,
and by the allocator to block rotation into illiquid sleeves during turbulent regimes (PH1).
"""
...
class SleeveDef:
    ...
    description: str
    etfs: tuple[ETFDef, ...]
    primary_ticker: str  # Default ETF used when only one ticker needed
