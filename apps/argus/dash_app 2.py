...
import dash
from dash import (
    Dash, dcc, html, callback, callback_context,
...
def _load_features9() -> pd.DataFrame:
    ...
    FEAT9 = [
        "country_exposure", "sector_exposure", "expense_ratio",
        "volatility_252d", "momentum_126d", "liquidity", "yield",
        "corr_to_spy", "asset_class_equity",
    ]
    return df[FEAT9]
...
def _load_raw_features() -> pd.DataFrame:
    ...
    SLEEVE_EXPENSE_RATIOS = {
        "SPY": 0.0945, "QQQ": 0.20, "XLF": 0.09,
        "GLD": 0.40, "SLV": 0.50,
        "SHY": 0.15, "SHV": 0.15, "IEF": 0.15, "IEI": 0.15,
        "TLT": 0.15, "TLH": 0.15, "LQD": 0.14, "VCIT": 0.04,
        "VNQ": 0.12, "IYR": 0.39,
        "DJP": 0.70, "GSG": 0.75, "DBC": 0.87,
        "VYM": 0.06, "DVY": 0.38, "SDY": 0.35,
        "VWO": 0.08, "EEM": 0.68,
    }
    COUNTRY_EXPOSURE = {
        "VTI": 1.0, "VOO": 1.0, "SPY": 1.0, "QQQ": 1.0,
        "IWM": 1.0, "VIG": 1.0, "SCHD": 1.0, "ESGU": 1.0,
        "XLK": 1.0, "XLF": 1.0, "XLE": 1.0, "XLV": 1.0, "XLI": 1.0,
        "VEA": 0.0, "EFA": 0.0, "VWO": 0.0, "EEM": 0.0,
        "BND": 1.0, "AGG": 1.0, "TLT": 1.0, "LQD": 1.0,
        "GLD": 1.0, "SLV": 1.0,
    }
    SECTOR_EXPOSURE = {
        "VTI": 0.3, "VOO": 0.3, "SPY": 0.3,
        "QQQ": 0.5, "XLK": 0.9,
        "IWM": 0.4, "VIG": 0.4, "SCHD": 0.4,
        "VEA": 0.0, "VWO": 0.0, "EEM": 0.0, "EFA": 0.0,
        "XLF": 0.8, "XLE": 0.9, "XLV": 0.9, "XLI": 0.9,
        "BND": 0.0, "AGG": 0.0, "TLT": 0.0, "LQD": 0.0,
        "GLD": 0.0, "SLV": 0.0, "ESGU": 0.3,
    }
    ...
    for ticker in tickers:
        ...
        raw["volatility_252d"].append(float(np.std(rets) * np.sqrt(252)) if len(rets) >= 20 else 0.0)
        if len(rets) >= 126:
...
def _build_xray_panel(ticker: str) -> html.Div:
    ...
    for col, label in [
        ...
        if val > 0:
            country_rows.append(
                html.Tr([html.Td(label), html.Td(f"{val:.1f}%"),
                         html.Td(_make_bar_cell(val))])
            )
    ...
    if holdings_error:
        ...
        if top_holdings is not None and not top_holdings.empty:
            ...
            for symbol, row_h in top_holdings.iterrows():
                ...
                hold_rows.append(
                    html.Tr([
                        ...
                        html.Td(f"{pct:.2f}%", style={"fontSize": "12px", "textAlign": "right"}),
                    ])
                )
            ...
            sector_label_map = {
                ...
                "consumer_cyclical": "Consumer", "industrials": "Industrials",
                "basic_materials": "Materials", "realestate": "Real Estate",
                "utilities": "Utilities", "communication_services": "Comm.",
                ...
                if pct_v > 0:
                    sector_rows_h.append(
                        html.Tr([
                            ...
                            html.Td(f"{pct_v:.1f}%", style={"fontSize": "11px", "textAlign": "right"}),
            ...
            ) if sector_rows_h else html.Small("No sector data", style={"color": "#999"})
                ...
                [
                    ...
                    dbc.Col(
                        [
                            ...
                            dbc.ListGroup(
                                [
                                    dbc.ListGroupItem(f"Expense Ratio: {float(row.get('expense_ratio', 0)):.2f}%"),
                                    dbc.ListGroupItem(f"Dividend Yield: {float(row.get('dividend_yield', 0)):.2f}%"),
                                    dbc.ListGroupItem(f"AUM: ${float(row.get('aum_b', 0)):.0f}B"),
                                    dbc.ListGroupItem(f"Avg Volume: {int(float(row.get('avg_volume', 0))):,}"),
                                ],
                                size="sm",
                            ),
                            ...
                            html.Div(
                                html.Details(
                                    [
                                        html.Summary(
                                            "View Holdings",
                                            style={"cursor": "pointer", "list-style": "none", "fontSize": "12px", "color": "#007bff"},
                                        ),
...
def _load_cluster_labels(k: int = 5) -> dict[str, int]:
    ...
    return dict(zip(data["tickers"], labels))
...
for preset_data in _presets["presets"].values():
    ...
    for cid in impl.get("hard_countries", {}).keys():
        if cid not in ALL_HARD_COUNTRY_IDS:
            ALL_HARD_COUNTRY_IDS.append(cid)
    for cid in impl.get("soft_countries", {}).keys():
...
for preset_data in _presets["presets"].values():
    for cat_id in preset_data.get("_impl", {}).get("categories", {}).keys():
...
def check_etf_compliance(
    ...
    country_overrides = {
        k: v for k, v in preset_override_keys.items()
        if not k.startswith("cat_") and k not in ("country_exposure",)
    ...
    if categories:
        ...
        for cat_id, cat_def in categories.items():
            ...
            if cat_overrides:
                enabled = cat_overrides.get(cat_id, {}).get("enabled", cat_def.get("default", False))
            else:
    ...
    if rule_type == "feature_threshold":
        ...
        if ticker in _raw_feats.index:
            ...
            passed = (operator == "<=" and value <= threshold) or (operator == ">=" and value >= threshold)
            if passed:
...
def compute_portfolio_compliance(
    ...
    if rule_type == "portfolio_level":
        ...
        yellow_label = impl.get("yellow_label", "Portfolio unbalanced")
        ...
        if cluster_pct >= threshold:
            ...
            portfolio_detail = f"{yellow_detail} ({cluster_count}/{len(ticker_list)} = {cluster_pct:.0%})"
...
def cosine_sim_in_cluster(ticker: str, top_n: int = 3) -> list[dict]:
    ...
    same_cluster_idx = [
        ...
        if t != ticker and _cluster_labels.get(t) == cluster
    ]
    ...
    ranked = sorted(zip(peer_tickers, sims), key=lambda x: x[1], reverse=True)[:top_n]
    ...
    [
        ...
        dbc.Row(
            dbc.Col(
                dbc.ButtonGroup(
                    [
                        dbc.Button("1. Compliance Editor", id="btn-page-1", outline=True, active=True),
...
def page1_layout(
    ...
    if portfolio:
        ...
        value_score, count_score, green_count, total_count = compliance_score(portfolio, active_preset, overrides)
        score_color = "success" if value_score >= 80 else "warning" if value_score >= 50 else "danger"
            ...
            rows.append(
                html.Tr(
                    [
                                ...
                                [
                                    html.Summary(
                                        [
                                            ...
                                            "  ",
                                            html.Small(
                                                _universe.loc[tkr, "name"] if tkr in _universe.index else tkr,
                                                style={"color": "#666"},
                                            ),
                                        ],
                        ...
                        html.Td(str(int(qty)) if qty else "—", style={"background": bg, "textAlign": "right"}),
                        html.Td(f"${mv:,.0f}" if mv else "—", style={"background": bg, "textAlign": "right"}),
                        html.Td(f"{weight:.1f}%" if total_mv > 0 else "—", style={"background": bg, "textAlign": "right"}),
                        html.Td(CLUSTER_NAMES.get(get_cluster(tkr), "—"), style={"background": bg}),
                        html.Td(
                            dbc.Badge(
                                st.upper(),
                                color={"green": "success", "yellow": "warning", "red": "danger"}[st],
                            ),
                            style={"background": bg},
                        ),
...
def page2_layout(portfolio: Optional[list] = None, preset_id: str = "none") -> html.Div:
    portfolio = portfolio or []
    ...
    for _, row in coords_df.iterrows():
        ...
        if t in portfolio_tickers:
            ...
            hover_texts.append(
                f"{t}: {status.upper()} — {comp[t]['detail']}"
            )
        else:
...
def page3_layout(
    ...
    if not portfolio:
        return html.Div(
            dbc.Alert("Add ETFs to your portfolio on the Compliance Editor page first.", color="info")
        )
    ...
    for ticker in violating_tickers:
        ...
        for rec in recs:
            rec_tkr = rec["ticker"]
            rec_mv = ticker_mv.get(rec_tkr, 0)
            swap_value = mv  # replacing violating with compliant removes this much violating exposure
            swap_desc = f"Replacing {ticker} with {rec_tkr} resolves SGD {swap_value:,.0f}" if swap_value > 0 else ""
            rec_rows.append(
                html.Tr(
                    [
                        ...
                        html.Td(html.Small(swap_desc, style={"color": "#155724"})) if swap_desc else html.Td(""),
                    ]
                )
            )
        ...
        rec_table = dbc.Table(
            [
                html.Thead(html.Tr(
                    [html.Th(h) for h in ["Ticker", "Name", "Cluster", "Cosine Sim", "Key Difference", "Swap Effect"]]
                )),
                html.Tbody(rec_rows),
            ],
            bordered=True,
            ...
            dbc.Card(
                dbc.CardBody(
                    [
                                ...
                                dbc.Col(
                                    html.H5(
                                        [
                                            ticker,
                                            " — ",
                                            _universe.loc[ticker, "name"] if ticker in _universe.index else ticker,
                                        ]
                                    ),
                                    width="auto",
                                ),
                        ...
                        html.Small(impact_desc, style={"color": "#856404"}) if impact_desc else html.Div(),
                        html.Hr(),
...
def _get_constraint_rows(
    ...
    if country_hard or country_soft:
        ...
        if override:
            ...
            threshold = country_hard.get("min", 0.0) * 100 if country_hard else country_soft.get("min", 0.0) * 100
        ...
        [
            ...
            html.Div(
                [
                    ...
                    html.Small(
                        f"{int(threshold)}%",
                        style={"width": 28, "textAlign": "center", "flexShrink": 0,
...
def build_constraint_panel(
    ...
    preset_name = preset_data.get("name", active_preset)
    ...
    for const_id, override_val in preset_overrides.items():
        ...
        default_type = "hard" if const_id in sector_hard_max or const_id == "country_exposure" and country_hard else "soft"
                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            dbc.Badge("HARD" if is_hard else "SOFT", color=badge_color,
                                       style={"marginLeft": 6, "fontSize": "10px"}),
                            html.Small(f" — {description}" if description else "",
                                       style={"color": "#666", "marginLeft": 4}),
                            html.Small(f" → {affected_str}" if affected_str else "",
                                       style={"color": "#999", "marginLeft": 4}),
                            html.Small(f" ({note})" if note else "",
...
def _build_geopolitical_countries_panel(active_preset: str, constraint_overrides: dict) -> list:
    ...
    if hard_countries:
        rows.append(
            html.H6("Hard Constraints — Zero Tolerance", style={"color": "#dc3545", "marginTop": 4, "marginBottom": 6})
        )
        for country_id, cdef in hard_countries.items():
            label = cdef.get("label", country_id.title())
            ...
            rows.append(
                html.Div(
                    [
                        ...
                        dbc.Badge("HARD", color="danger", style={"marginLeft": 6, "fontSize": "10px"}),
                    ],
    ...
    if soft_countries:
        ...
        rows.append(
            html.H6("Soft Constraints — Warning Threshold", style={"color": "#ffc107", "marginTop": 4, "marginBottom": 6})
        )
        for country_id, cdef in soft_countries.items():
            label = cdef.get("label", country_id.title())
            ...
            rows.append(
                html.Div(
                    [
                        ...
                        dbc.Badge("SOFT", color="warning", style={"marginLeft": 6, "fontSize": "10px"}),
                        html.Div(
                            [
                                html.Label(
                                    f"Max: {threshold:.0f}%",
                                    id={"type": "soft-country-threshold-label", "index": country_id},
...
def _update_constraint_override(
    slider_values, hard_btn_clicks, soft_btn_clicks, cat_checkbox_values, cat_toggle_clicks,
    hard_country_values, soft_country_values, soft_slider_values,
    ...
    overrides[active_preset] = dict(overrides.get(active_preset, {})) if overrides.get(active_preset) else {}
        ...
        if constraint_id in ALL_SOFT_COUNTRY_IDS and soft_country_values is not None:
            ...
            if idx < len(soft_country_values):
                ...
                overrides[active_preset][f"soft_{constraint_id}"] = {
                    "enabled": enabled,
                    "threshold": overrides[active_preset].get(f"soft_{constraint_id}", {}).get("threshold", 10.0),
                }
            ...
            if cat_idx < len(cat_toggle_clicks) and cat_toggle_clicks[cat_idx]:
                ...
                if constraint_id in cat_defs:
                    ...
                    current_enabled = current.get(
                        "enabled",
                        cat_defs.get(constraint_id, {}).get("default", False)
                    )
                    new_enabled = not current_enabled
                    overrides.setdefault(active_preset, {})[f"cat_{constraint_id}"] = {"enabled": new_enabled}
...
def _build_preset_panel(active_preset: str) -> html.Div:
    ...
    CATEGORY_ORDER = ["Ethical & ESG", "Geopolitical", "Risk & Cost", "Sector Focus", "Portfolio Structure"]
    ...
    for cat in CATEGORY_ORDER:
        ...
        preset_sections.append(
            html.Div(
                [
                    html.H6(cat, style={"marginBottom": 4, "marginTop": 8, "fontSize": "11px", "color": "#888", "textTransform": "uppercase"}),
    ...
    if uncategorized:
        preset_sections.append(
            html.Div(
                [
                    html.H6("Other", style={"marginBottom": 4, "marginTop": 8, "fontSize": "11px", "color": "#888"}),
                    html.Div(uncategorized, style={"display": "flex", "flexWrap": "wrap", "gap": "2px"}),
                ]
            )
        )
        ...
        rationale_rows = [
            ...
            html.Div(
                [
                    ...
                    html.Span(rat.get("roadmap", ""), style={"color": "#888", "fontSize": "11px", "fontStyle": "italic"}),
                ],
            ),
        ]
    elif active_preset != "none":
        rationale_rows = [html.Hr(), html.Small("No rationale available for this preset.", style={"color": "#aaa"})]
...
def _build_portfolio_panel(portfolio: list, feedback: list) -> html.Div:
    ...
    for msg in feedback:
        feedback_rows.append(
            dbc.Alert(msg, color="warning", dismissable=True, style={"fontSize": "12px", "padding": "4px 8px"})
        )
    ...
    return html.Div(
        [
            ...
            dcc.Upload(
                id="upload-csv",
                children=dbc.Button("Upload CSV", id="upload-csv-btn", color="outline-primary", size="sm"),
                accept=".csv",
...
def _parse_csv_contents(contents: str) -> Optional[dict]:
...
def _load_sample_csv() -> Optional[dict]:
...
def handle_csv_import(contents, sample_clicks, current_portfolio):
    ...
    if result is None:
        return {"positions": [], "imported": False, "log": "Failed to parse CSV"}, current_portfolio or [], dbc.Alert("Failed to parse CSV file", color="danger", dismissable=True)
    ...
    if not in_universe:
        log_msg = dbc.Alert(
            ...
            color="warning", dismissable=True, style={"fontSize": "12px", "padding": "4px 8px"}
        )
        return {"positions": [], "imported": False, "log": str(out_of_universe)}, current_portfolio or [], log_msg
    ...
    log_msg = dbc.Alert(
        " ".join(log_parts),
        color="success", dismissable=True, style={"fontSize": "12px", "padding": "4px 8px"}
    )
    return (
